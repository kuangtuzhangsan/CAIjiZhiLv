...
//
// Related Topics 数学 👍 2759 👎 0
public class it1225 {
    public static boolean isPalindrome(int x) {
        if (x < 0 || (x /10 > 0 && x % 10 == 0) ){
            return false;
        }
        int coureversedNumbernt = 0;
        while (coureversedNumbernt < x){
            int digit = x%10;
            x = x/10;
            coureversedNumbernt = coureversedNumbernt*10 + digit;
        }
        if (x == coureversedNumbernt || x == coureversedNumbernt/10){
            return true;
        }
        return false;
